...
async function netId(display) {
    ...
    try {
        const nId = pTimeout(promisify(web3.version.getNetwork)(),WEB3_TIMEOUT);
        if (display) console.log('Attached to network:',nId);
    }
    catch(err) {
...
async function stopWatchProvider() {
...
async function startProviderWatcher() {
    let account;
        ...
        try {
            ...
            if (LOG_TRANSACTIONS.length) {
                for (let log of LOG_TRANSACTIONS) {
                    web3Provider.on(log, msg => console.log('Eth log',(Array.isArray(log) ? 'topics' : ''),(log.length ? log : '*'),' -> ',msg));
                }
            }
        }
        catch(err) {
